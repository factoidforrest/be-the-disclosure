...
export default class DelugeHandler {
  ...
  /**
     ...
     */
  constructor (deluge_url: string, password: string, directory: string, cookies?: Record<string, string>) {
    if (directory !== '' && deluge_url !== '' && password !== '') {
      this.DELUGE_URL = deluge_url;
      this.PASSWORD = password;
      this.DOWNLOAD_PATH = directory;
      if (cookies) { this.COOKIE_JAR = cookies; }
    } else { throw new Error('host url and password must be defined'); }
  }

  /**
     * @desc Get the list of all the hosts that the WebUI can connect to
     */
  async getHosts (): Promise<Array<{ id: string, ip: string, port: number, status: string }> | null> {
  ...
  async add (magnet: string, location?: string) {
    if (validUrl.isWebUri(magnet)) {
      ...
      const response = await this.secondCalls('web.download_torrent_from_url', [magnet, cookie]);
      if (response) { await this.addTorrent(response.result, location); }
    } else { await this.addTorrent(magnet, location); }
  }
  ...
  private async secondCalls (method: string, params: any[]) {
    ...
    return response?.data && response.data.error === null ? response.data : null;
  }
